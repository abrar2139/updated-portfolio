...
export const Contactcomponent = () => {
  const [status, setStatus] = useState("Send")
    ...
    let details = {
      ...
      let response = await fetch("https://mvc-nodemailer.vercel.app/send", {
        mode:"cors",
        method:"POST",
        headers:{
  ...
  return (
    <section id='contact' className="lg:h-screen  md:h-screen xl:h-screen h-auto sm:h-screen text-gray-600 bg-slate-600 body-font lg:flex lg:items-center md:flex md:items-center relative ">
    <div className="px-5 py-5 pt-20  mx-auto flex sm:flex-nowrap flex-wrap justify-center">
      ...
      <form  onSubmit={sendEmail} className="lg:w-1/3 md:w-1/2 bg-slate-200  flex mb-24 flex-col md:ml-auto w-full p-1 mt-4 md:mt-0 rounded-lg">
        ...
        <div className="relative mb-4">
          ...
          <input required placeholder='Type Your Name' type="text" id="name" name="from_name" className="w-full bg-white rounded-xl border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
        </div>
        <div className="relative mb-4">
          <label htmlFor="email" className="leading-3 text-1xl font-semibold text-amber-600 hover:text-slate-600">Email</label>
          <input placeholder='Type Your Email Id' required  type="email" id="email" name="user_email" className="w-full bg-white rounded-xl border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
        </div>
        <div className="relative mb-4">
